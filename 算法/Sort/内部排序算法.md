...
private void quickSortHelper(int[] nums, int low, int high) {
    if (low < high) { // low == high表示子数组只有一个元素，不用继续递归
        int pivotPos = partition(nums, low, high);
...
private void buildMaxHeap(int[] nums) {
    for (int parent = (nums.length - 2) / 2; parent >= 0; parent--) {
        // 建立初始最大堆的时候，待排序列的长度恒为nums.length
...
private void percolateDown(int[] nums, int lenOfUnsorted, int parent) {
    int tmp = nums[parent];
    // parent * 2 + 1为parent左孩子的下标
    for (int child = parent * 2 + 1; child <= lenOfUnsorted - 1; child = child * 2 + 1) {
        // 如果当前遍历到的节点有右孩子，并且右孩子大于左孩子的值
        if (child - 1 < lenOfUnsorted && nums[child] < nums[child + 1]) {
            child++;
        }
...
public void heapSort(int[] nums) {
    ...
    buildMaxHeap(nums);
    // start为下一个堆顶元素在有序序列中放置的位置，即有序队列的起始位置，也待排序列的长度
    for (int start = nums.length - 1; start > 0; start--) {
        swap(nums, start, 0); // swap在percolate之前是因为for循环之前已经是一个最大堆了
        percolateDown(nums, start, 0);
    }
}
```
